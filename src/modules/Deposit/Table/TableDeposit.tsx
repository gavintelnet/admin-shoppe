...
import { getAllDeposit } from "../apis";
  ...
  useEffect(() => {
    const getList = async () => {
      ...
      try {
        ...
        const response = await getAllDeposit(payload);
        if (response.status) {
          const updatedUsers: any = response.result.deposits.map(
            (item: any, i: any) => ({
              ...item,
