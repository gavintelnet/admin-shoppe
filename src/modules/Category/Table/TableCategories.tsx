...
const TableCategories = (props: any) => {
  ...
  const showModal = async (id: string) => {
    ...
    try {
      const rp = await getDetailCategory(id);
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      const response = await updateCategory(idDataDetail, payload);
      if (response.status) {
        dispatch(
          showNotification({
  ...
  useEffect(() => {
    const getList = async () => {
      ...
      try {
        ...
        if (response.status) {
          const updatedUsers: any = response.result.categories.map(
            (item: any, i: any) => ({
              ...item,
